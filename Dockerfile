...
LABEL version=4.14.6

COPY config.patch /opt/atlassian/stash/

RUN apt-get update && \
    apt-get install --assume-yes --no-install-recommends wget patch && \
    echo "start downloading and decompressing https://www.atlassian.com/software/stash/downloads/binary/atlassian-bitbucket-4.14.6.tar.gz" && \
    wget -q -O - https://www.atlassian.com/software/stash/downloads/binary/atlassian-bitbucket-4.14.6.tar.gz | tar -xz --strip=1 -C /opt/atlassian/stash && \
